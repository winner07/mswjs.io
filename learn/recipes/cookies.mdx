...
On the "server" side, cookies are handled by the library to ensure that they are sent to any request made within your response resolver. The library respects a cookie's domain and expiration date when doing so.
