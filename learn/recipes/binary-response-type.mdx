...
Providing the `ctx.body()` utility function with a [`BufferSource`](https://developer.mozilla.org/en-US/docs/Web/API/BufferSource) object will use that buffer as the mocked response's body. Support of binary data allows sending various media content, such as images, audio, documents, in a mocked response.
...
const server = setupServer(
  rest.get('/images/:imageId', (_, res, ctx) => {
    ...
    return res(
      // Set the correct binary type and length.
      ctx.set('Content-Type', 'image/jpeg'),
      ctx.set('Content-Length', imageBuffer.byteLength.toString()),
      ctx.body(imageBuffer),
    )
