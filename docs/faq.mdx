...
Be it `axios`, `react-query`, or plain `fetch`—Mock Service Worker supports all libraries that issue requests in both browser and Node.js.
...
Yes, but only for Node.js integration.
...
> In fact, Mock Service Worker ignores any query parameters present in the URL. To reduce the clutter and make your request handlers easier to navigate, the library produces a warning message to remove the query parameters.
