...
Request matching substantially differs based on the request handler that's being used (`rest` or `graphql`). See the summary of that difference between the standard request handlers below.
...
You can test how Mock Service Worker resolves paths against the actual request URL using the [Path matching REPL](/repl).
...
Working with `graphql.*` request handlers allows you to match GraphQL operations by the following criteria:

- Operation kind (query or mutation);
- Operation name.

> GraphQL operations made via both `GET` and `POST` methods are supported.
