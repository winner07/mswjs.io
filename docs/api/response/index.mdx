...
Unlike the `Response` object in Node.js, the `res` argument in Mock Service Worker is used to create and transform a mocked response. When called, it passes a created response from one [response transformer](/docs/basics/response-transformer) to another, eventually resolving to a mocked response used by the Service Worker.
